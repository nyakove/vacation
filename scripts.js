...
function addPersons() {
    if (localStorage.length == 0 || JSON.parse(localStorage.getItem('persons')).length != main.user.options.length) {
        for (let i = 0; i < main.user.options.length; i++) {
            persons.push(new Person(main.user.options[i].text, 30, 'male')) //fix age and sex later
        }
        localStorage.clear();
